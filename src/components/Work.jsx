...
<h1 className="text-[28vw] md:text-[31vw] leading-none font-medium select-none tracking-tight">
  work
</h1>
<div className="absolute top-0 w-full h-full">
  {images.map((elem, index) =>
    elem.isActive && (
      <img
        key={index}
        className="absolute rounded-lg -translate-x-1/2 -translate-y-2/5 w-32 sm:w-40 md:w-56 transition-all duration-300"
        // On md and above, w-56 (original size); on mobile, w-32/sm:w-40
        src={elem.url}
        style={{
          top: elem.top,
          left: elem.left,
